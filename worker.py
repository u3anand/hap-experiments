...
def run(global_rank, local_rank, device_flops, communication_bandwidth, config):
    import hap
...
if __name__ == '__main__':
    ...
    device_flops = get_device_flops_for_machine(main_args.machine, config.model_name, config.batch_size)
    communication_bandwidth = get_comm_bandwidth_for_machine(main_args.machine)
    
    import torch.multiprocessing as mp
    mp.set_start_method('spawn')

    for local_rank, global_rank in enumerate(main_args.ranks):
        mp.Process(target=run, args=(global_rank, local_rank, device_flops, communication_bandwidth, config)).start()
